...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode; }>) {
  ...
  const navigation = [
    { icon: <HomeIcon className="size-4 inline" />, name: 'Home', href: '/', current: (currentPath == "/") },
    { icon: <CodeBracketSquareIcon className="size-4 inline" />, name: 'Models', href: '/entities', current: (currentPath.startsWith("/entities")) },
    { icon: <AdjustmentsHorizontalIcon className="size-4 inline" />, name: 'Settings', href: '/settings', current: (currentPath.startsWith("/settings")) },
  ]
            ...
            <div className="flex h-16 items-center justify-between">
                <div className="flex items-center">
                  <div className="flex-shrink-0">
                    ...
                    <h1 className="text-xl text-white font-black">IronERP</h1>
                  </div>
                  <div className="hidden md:block">
                    <div className="ml-5 flex items-baseline space-x-4">
                      {navigation.map((item) => (
                        <a key={item.name} href={item.href} aria-current={item.current ? 'page' : undefined} className={classNames(item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium', )}>
                          {item.icon} {item.name}
                        </a>
                      ))}
                    </div>
